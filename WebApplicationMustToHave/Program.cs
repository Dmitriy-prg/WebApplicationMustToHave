...
//    {
//        await db.compositions.AddAsync(new DbComposition { Id = 1, Name = "Аватар", YearBirth = 2009, DbCompositionTypeId = 3 });
//        await db.compositions.AddAsync(new DbComposition { Id = 2, Name = "Начало", YearBirth = 2010, DbCompositionTypeId = 3 });
//        await db.compositions.AddAsync(new DbComposition { Id = 3, Name = "Тайные виды на гору Фудзи", YearBirth = 2018, DbCompositionTypeId = 1 });
...
//    // если произведение не найдено, отправляем статусный код и сообщение об ошибке
//    if (composition == null) return Results.NotFound(new { message = "Произвекдение не найдено" });

//    // если произведение найдено, отправляем его
